...
export const bookingsSlice = createSlice({
    ...
    extraReducers(builder){
        ...
        .addCase(fetchBookings.fulfilled, (state: BookingsState, action: BookingsAction) =>{
            state.bookingListData = action.payload;
            state.status = "fulfilled";
            
        })

        .addCase(addBooking.fulfilled, (state: BookingsState, action: BookingsAction) =>{
        ...
        .addCase(getBooking.fulfilled, (state, action: BookingsAction) =>{
            if(typeof action.payload === "object"){
                ...
                if(state.bookingListData !== undefined){
                    state.singleBookingData = state.bookingListData.find((booking : Booking) => booking.id === action.payload)
                } 
            }
